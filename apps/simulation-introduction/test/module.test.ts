...
import { moduleConfig } from "../src/components/SimulationIntroductionApp/module-config";
import { createDefaultControls, createState } from "../src/components/SimulationIntroductionApp/model";
import { runExample } from "../src/simulation/engine";

describe("Simulation Introduction", () => {
  it("keeps the migrated WALS source scope in module metadata", () => {
    expect(moduleConfig.sourcePath).toBe("apps/Simulation/Introduction");
    expect(moduleConfig.examples.length).toBe(2);
  });
