...
import { moduleConfig } from "../src/components/SimulationRandomVariableApp/module-config";
import { createDefaultControls, createState } from "../src/components/SimulationRandomVariableApp/model";
import { runExample } from "../src/simulation/engine";

describe("Random Variable Generation", () => {
  it("keeps the migrated WALS source scope in module metadata", () => {
    expect(moduleConfig.sourcePath).toBe("apps/Simulation/RandomVariable");
    expect(moduleConfig.examples.length).toBe(3);
  });
